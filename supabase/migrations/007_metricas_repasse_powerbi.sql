-- Borderô: estritamente Data Inclusão no intervalo.
-- Recebimentos: Data Repasse no intervalo (carteira inteira).
...
BEGIN
  ...
  pagamentos AS (
    ...
    WHERE credor_id = p_credor_id
      ...
      AND data_repasse BETWEEN p_data_inicio AND p_data_fim
      AND (p_sistema IS NULL OR sistema_origem = p_sistema)
  )
  SELECT jsonb_build_object(
    'bordero_total', COALESCE((SELECT SUM(valor_original_titulo) FROM bordero_linhas), 0),
    'bordero_qtd', COALESCE((SELECT COUNT(DISTINCT processo) FROM bordero_linhas), 0),
    ...
    'pagamento_direto', COALESCE((
      ...
      WHERE tipo IN ('Pagamento Direto Final', 'Pagamento Direto Parcial')
         OR (status ILIKE 'Pagto. Direto' AND (tipo IS NULL OR btrim(tipo) = ''))
    ), 0),
